...
def resolve_wa_name(num):
    ...
    result = (
        conn.cursor()
        .execute("SELECT wa_name FROM wa_contacts WHERE jid=?", (num,))
        .fetchone()
    )
...
if os.path.exists(DB_FILE_AXOLOTL) and os.path.exists(DB_FILE_WA):
    ...
    status_cands = set(
        [
            ...
            .execute(
                'SELECT sender_id FROM sender_keys WHERE group_id="status@broadcast"'
            )
            .fetchall()
        ]
    )
    ...
    listed_contacts = set(
        [
            str(x[0].strip("@s.whatsapp.net"))
            for x in conn.cursor()
            .execute("SELECT jid FROM wa_contacts WHERE sort_name IS NOT NULL")
            .fetchall()
        ]
    )
    ...
    print(
        "axolotl.db and/or wa.db file not found, place in current directory or specify in DB_DIR"
    )
